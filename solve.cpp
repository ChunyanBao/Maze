...
//	info(point P, int mycost, info * parent)
...
*/
void update_dijkstra(Maze &m,shared_ptr<info> currentInfo, int dir, priority_queue<shared_ptr<info>, vector<shared_ptr<info>>, compare> & pq, unordered_map<point, int, pair_hash> & best)
{
	point p1 = currentInfo->p + moveIn(dir);
	int newCost = currentInfo->cost + m.cost(currentInfo->p.first, currentInfo->p.second, dir);
...

{
	...
	//the starting point and target point
	//point start(0, 0);
	//point end(rows - 1, cols - 1);
	...
	//info *startPoint = new info(start, 0, NULL);
	//shared_ptr<info> startPoint = make_shared<info> (start, 0, NULL);
